...
int    CMD_IN   = 0;
int    i_LVL    = 0;
int    ii_LVL[max_nn] = {0,0,0,0,0,0,0,0,0};
float  LIn_val  = 0.;
...
unsigned long ComBaudSlv3 =9600L;
int Com_lgthSlv3  =8;
int Com_paritySlv3=0;// None  //0-(None);1-(Even);2-(Odd);3-(MARK, always 1); 4-(SPACE,always 0)
int Com_stopSlv3  =1;
...
&CMD_OUT      ,   // 9   данные с нижнего контроллера N1
&Reg_State    ,   // 10  данные с нижнего контроллера N1
&Reg_err      ,   // 11  данные с нижнего контроллера N1
&i_LVL        ,   // 12  данные в нижний контроллер   N1,N2
...
long int Com_Baud_Slave3=9600;
long int *list_Slv2_lng[]={
                    // Addr = 1042
&Com_Baud_Slave2,    //  21  Частота обмена по Modbus
&Com_Baud_Slave3,    //  22  Частота обмена по Modbus
...
// Addr2Slot1 2000
//  данные, читаемые нижними контроллерами
//
...
int Slv2_short_lgth= 13 ;
int Slv2_short_max=sizeof(Slv2_Short) / sizeof(S1D) ;

//---------------------------------------
// Addr2Slot2   3000
// данные, записываемые нижним контроллером N1
...
int Slv2_slt4_lgth=  19;
int Slv2_slt4_max=sizeof(Slv2_Slot4) / sizeof(S1D) ;
//---------------------------------------
// Addr2Slot5    6000
// данные, записываемые нижним контроллером N1
struct eee_dat Slv2_Slot5[]=
{
&s_MVD[2].MassI       , T_FLOAT ,
&s_MVD[2].MassT       , T_FLOAT ,  //   MassT2_tmp
&s_MVD[2].VolI        , T_FLOAT ,
&s_MVD[2].VolT        , T_FLOAT ,
&s_MVD[2].FlowM       , T_FLOAT ,
&s_MVD[2].Dens        , T_FLOAT ,
&s_MVD[2].Temp        , T_FLOAT ,
&Dns3                 , T_FLOAT , //&s_DGM[2].DensReal
&TTmp3                 , T_FLOAT , //&s_DGM[2].TempReal
&PMP_sw        , T_INT   ,        //1
};
int Slv2_slt5_lgth=  19;
int Slv2_slt5_max=sizeof(Slv2_Slot5) / sizeof(S1D) ;
//---------------------------------------
// Addr2Slot6    7000
// данные, записываемые нижним контроллером N2
struct eee_dat Slv2_Slot6[]=
{
&CMD_OUT2            , T_INT   ,
&Reg_State2          , T_INT   ,
&Reg_err2            , T_INT   ,
...
&OUT_VAR2            , T_INT   ,
&INP_VAR2            , T_INT   ,
&s_MVD[3].MassI       , T_FLOAT ,
&s_MVD[3].MassT       , T_FLOAT ,
&s_MVD[3].VolI        , T_FLOAT ,
&s_MVD[3].VolT        , T_FLOAT ,
&s_MVD[3].FlowM       , T_FLOAT ,
&s_MVD[3].Dens        , T_FLOAT ,
&s_MVD[3].Temp        , T_FLOAT ,
&RQ2                   , T_INT   ,
};
int Slv2_slt6_lgth=  20;
int Slv2_slt6_max=sizeof(Slv2_Slot6) / sizeof(S1D) ;
//---------------------------------------
...
// вводит команду из Host (ICP CON) и интерпретирует ее по протоколу Modbus RTU
         ...
         if(ffgets_Slv2RTU(ComPortSlv3,AddrSlv3) )
         {
          _fmemcpy(intrpr.bfcom,&cb_COM[ComPortSlv3][0],(long int)(n_bcom[ComPortSlv3]-2) );
          n_bcom[ComPortSlv3]=0;
          f_intr_Slv2RTU(ComPortSlv3,AddrSlv3);
         }
}
/*-------------------------------------------*/
/*-------------------------------------------*/
...
m_slt:
     ...
     else if((Slv_addrO == Addr2Slot12 ) && ( Slv_nmbO <= Slv2_slt1_lgth))
     {
      LIn_D=&Slv2_Slot1[0];
      goto m_slt;
     }
...
mm20:
    ...
    else if((Slv_addrO == Addr2Slot5 ) && ( Slv_nmbO <= Slv2_slt5_lgth))
    {  // Slot5
      if(f_set_slv( &Slv2_Slot5[0],Slv_nmbO )) goto m_err_val;
      ...
      if((FlagNewDT & 4) == 0)
      {
       ...
       if(Reg_State & DEN_fin )
       {
        if((s_DGM[2].DensReal != Dns3) ||
          (s_DGM[2].TempReal != TTmp3) )
          {
            s_DGM[2].DensReal=Dns3 ;
            s_DGM[2].TempReal=TTmp3 ;
    ...
    else if((Slv_addrO == Addr2Slot6 ) && ( Slv_nmbO <= Slv2_slt6_lgth))
    {  // Slot6
      if(f_set_slv( &Slv2_Slot6[0],Slv_nmbO )) goto m_err_val;
