...
char ScrBuf_c[48][31]; /* копи€ состо€ни€ экрана */
char mn_fnt='S';
...
//           зоны задаютс€ после смены страницы, указываетс€ начальна€ позици€,
//           длина и фонт,  функци€ SetZone(int zone,int x,int y,int lgth,int font);
...
int page_MMI_c=-1;   /* копи€ состо€ни€ страницы MMI  */
...
/*  отображает экранный буфер
 ...
 јнализ клавиш см. MmiKbhit(),MmiGetch().
*/
// char *buf,*buf_c;
 char *buf_c;
 char buf[32];
 ...
 if(page_MMI != page_MMI_c)
  {
   f_clr_scr_c_MMI(); // заполнение буфера копии пробелами
                      // т.е. фактическое состо€ние экрана после смены страницы
...
//printf("\r\n MMI page %d",page_MMI);
 ...
 {
 /* найти и отобразить изменени€  с самого начала буфера */
   fl_mmi_new1=1;
   fl_mmi_new=0;
   mmix=0;
   mmiy=0;
 ...
 {
 /*-------------------------*/
 // —равнение буфера экрана и вывод в MMI при наличии расхождений
...
/*----------------------------*/
// величина таймаута в мс дл€ получени€ ответа на команду выведенную в порт
unsigned long ToutAns[5]= {200,200,200,200,200};
unsigned int flag_fn=0;
int cnt_flt_MMI=0;
void SendToMMI()
/*
 ‘ункци€ с интервалом PoolKbd_MMI шлет команды чтени€  состо€ни€ клавиатуры (fn_MMI==1).
 ѕри наличии запроса, устанавливает требуемую страницу диспле€ 'page' (fn_MMI==2)
...
*/
{
 ...
 // есть что выводить и есть место в очереди запросов
      ...
      {
      // предыдуща€ посылка завершилась со сбоем,
      // стать в очередь доступа к COM с повтором команды
      // оставшейс€ в структуре MMI_Rqst, если это была
...
{
// ѕроизошел таймаут при приеме ответа.
// ќчистка очереди и статистика.
...
{
 // ќбработка ответа от MMI на команды изменени€ страницы
 // и записи символа(ов).Ќикаких действий не требуетс€.
 // ќсвобождение очереди и статистика результата.
...
/* при наличии нажатых клавиш, вводит коды клавиш в кольцевой буфер  mmikey[]
   ...
   mmi_key_in  - указывает на пустую €чейку дл€ ввода символа
...
/*
  MmiGotoxy(0,0);  MmiPrintf("Џƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒњ");//  0
  MmiGotoxy(0,1);  MmiPrintf("≥         ≥        ≥         ≥");//  1
  MmiGotoxy(0,2);  MmiPrintf("√ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒі");//  2
  MmiGotoxy(0,3);  MmiPrintf("≥         ≥        ≥         ≥");//  3
  MmiGotoxy(0,4);  MmiPrintf("≥         ≥        ≥         ≥");//  4
  MmiGotoxy(0,5);  MmiPrintf("√ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒі");//  5
  MmiGotoxy(0,6);  MmiPrintf("≥         ≥        ≥         ≥");//  6
  MmiGotoxy(0,7);  MmiPrintf("≥         ≥        ≥         ≥");//  7
  MmiGotoxy(0,8);  MmiPrintf("√ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒі");//  8
  MmiGotoxy(0,9 ); MmiPrintf("≥         ≥        ≥         ≥");//  9
  MmiGotoxy(0,10); MmiPrintf("≥         ≥        ≥         ≥");// 10
  MmiGotoxy(0,11); MmiPrintf("√ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒі");// 11
  MmiGotoxy(0,12); MmiPrintf("≥         ≥        ≥         ≥");// 12
  MmiGotoxy(0,13); MmiPrintf("≥         ≥        ≥         ≥");// 13
  MmiGotoxy(0,14); MmiPrintf("≥         ≥        ≥         ≥");// 14
  MmiGotoxy(0,15); MmiPrintf("јƒƒƒƒƒƒƒƒƒЅƒƒƒƒƒƒƒƒЅƒƒƒƒƒƒƒƒƒў");// 15
