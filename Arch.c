...
*/
  ...
  m_res:
    ...
    printf("\r\nРез%d.Уровень %6.1f мм;Плотн.=%6.1f; Темп.=%5.1f " ,i_res,a_str->u.fill.Amount,a_str->u.fill.Density,a_str->u.fill.Temp );  //%8.3f
    printf("\r\nV=%8.1f л;M общ.=%8.1f;Плотн.пар= %6.1f" ,a_str->VolI,a_str->MassI,a_str->u.fill.UnitPrice );
 ...
 mm_0:
  ...
  if(itmp)
  {
    printf(" Ошибка:");
 mm_e:
    printf("%01X" ,a_str->u.err.err[0] );     // MMI
    printf(" %01X" ,a_str->u.err.err[1] );    // MVD
    printf(" %01X" ,a_str->u.err.err[2] );    // TSD
    printf(" %01X" ,a_str->u.err.err[3] );    // WDOG
    printf(" %01X" ,a_str->u.err.err[4] );    // I7060[0]
    printf(" %01X" ,a_str->u.err.err[5] );    // I7017C[0]
    printf(" %01X" ,a_str->u.err.err[6] );   // Drive
    printf(" %01X" ,a_str->u.err.err[7] );    //  System
    printf(" %01X" ,a_str->u.err.err[8] );    // код
...
//  s_arch->u.fill.Amount     = amount_f;
    ...
    s_arch->u.fill.Amount     = 0.;//s_MVD[0].TempR;
    s_arch->u.fill.UnitPrice  = 0.;//s_MVD[0].Press;
    s_arch->u.fill.Density    = 0.;//s_MVD[0].Dens;
    s_arch->u.fill.Temp       = 0.;//s_MVD[0].Temp;
   break;

   case evt_store1:
    s_arch->u.fill.Amount     = s_DGM[0].Prod_lvl;
    s_arch->u.fill.UnitPrice  = s_DGM[0].DensGas;
    s_arch->u.fill.Density    = s_DGM[0].DensReal;
    s_arch->u.fill.Temp       = s_DGM[0].Temp;
    s_arch->u.fill.VolT       = 0;

    s_arch->VolI   = s_DGM[0].VolLiq;
    s_arch->MassI  = s_DGM[0].MassLiq;
    goto m_dte;

   case evt_store2:
    s_arch->u.fill.Amount     = s_DGM[1].Prod_lvl;
    s_arch->u.fill.UnitPrice  = s_DGM[1].DensGas;
    s_arch->u.fill.Density    = s_DGM[1].DensReal;
    s_arch->u.fill.Temp       = s_DGM[1].Temp;

    s_arch->VolI   = s_DGM[1].VolLiq;
    s_arch->MassI  = s_DGM[1].MassLiq;
    goto m_dte;

   case evt_store3:
    s_arch->u.fill.Amount     = s_DGM[2].Prod_lvl;
    s_arch->u.fill.UnitPrice  = s_DGM[2].DensGas;
    s_arch->u.fill.Density    = s_DGM[2].DensReal;
    s_arch->u.fill.Temp       = s_DGM[2].Temp;

    s_arch->VolI   = s_DGM[2].VolLiq;
    s_arch->MassI  = s_DGM[2].MassLiq;
    goto m_dte;
   ...
   case evt_wd_pmp :   // Таймаут связи
    ...
    s_arch->u.fill.VolT=0; //s_MVD[0].MassT;

    s_arch->u.fill.Amount     = 0; //s_MVD[0].VolT;;
    s_arch->u.fill.UnitPrice  = 0;  //s_MVD[0].Press;

    s_arch->u.fill.Density    = 0;//Dens_a;
    s_arch->u.fill.Temp       = 0;// Temp_a;
...
//    s_arch->u.fill.Temp       = Temperature1;
   ...
   break;
    ...
    s_arch->u.fill.VolT       = 0;//chg_nn_par;
    s_arch->u.fill.Amount     = 0;//chg_old;
    s_arch->u.fill.UnitPrice  = 0;//chg_new;
   break;
 ...
 m1:
  ...
  s_arch->u.err.err[9]  = ICP_error[12];
