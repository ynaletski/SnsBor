...
int i1;
    i1=ii+1;
//    if(res_ctr[ii] != 0)
MmiGotoxy(0,1);  MmiPrintf("  'ООО Факом Технолоджиз'");

////29.04.2021 YN -\\//-
if(waterTank == 1)
{
  MmiGotoxy(0,2);  MmiPrintf("   Рез-р авар.проливов 90%  ");
}
else
{
...
if(PMP[ii].status==2)
  {
    ...
    {
    MmiGotoxy(0,3);  MmiPrintf("       Уровнемер %dXX ",i1);
    }
    else
    {
    MmiGotoxy(0,3);  MmiPrintf("       Уровнемер %d  ",i1);
    }
    MmiGotoxy(0,5); MmiPrintf("Уровень продукта  мм:"); MmiGotoxy(21,5); MmiPrintf("%8.1f ",s_DGM[ii].Prod_lvl);
  }
else
  {
    if(( ( INP_VAR2) & ctr_msk[ii]) != 0)
...
//    MmiGotoxy(13,3);  MmiPrintf(" %8.1f л",s_DGM[ii].VolLiq);
	...
	MmiGotoxy(0,6); MmiPrintf("Уровень подт.воды мм:"); MmiGotoxy(21,6); MmiPrintf("%8.1f ",s_DGM[ii].Wat_lvl);
    MmiGotoxy(0,7); MmiPrintf("Плотность%d     кг/м3:",i1); MmiGotoxy(21,7); MmiPrintf("%8.1f ",s_DGM[ii].DensLiq);
	MmiGotoxy(0,8); MmiPrintf("Температура%d       С:",i1); MmiGotoxy(21,8); MmiPrintf("%8.1f ",s_DGM[ii].Temp);

    MmiGotoxy(0,9); MmiPrintf("Объем продукта%d    л: ",i1); MmiGotoxy(21,9); MmiPrintf("%8.1f ",s_DGM[ii].VolLiq);
    MmiGotoxy(0,10); MmiPrintf("Объем воды%d        л: ",i1); MmiGotoxy(21,10); MmiPrintf("%8.1f ",s_DGM[ii].VolWat );
    MmiGotoxy(0,11); MmiPrintf("Mасса продукта%d   кг: ",i1); MmiGotoxy(21,11); MmiPrintf("%8.1f ",s_DGM[ii].MassLiq);
...
MmiGotoxy(0,i1++); MmiPrintf("Авария оболочки резервуара");
    ...
    /*if(itmp & LVL_water)
    {
      MmiGotoxy(0,i1++); MmiPrintf("90% резервуар сбора проливов");
      if(i1 >= n_mmi_str) return;
    }*/
    ...
    {
MmiGotoxy(0,i1++); MmiPrintf("  Резервуар сбора проливов  ");
    if(i1 >= n_mmi_str) return;
    }
    //              -//\\-
...
if((flag_prn) && (key != 38 ))
{
  printf("\n\r key=%d   sw_mmi=%d   PageD=%d ",key, sw_mmi, PageD);
  //flag_prn=0;
}
...
if(flag_prn)
{
  printf("\n\r begin   key=%d   Tank_nn=%d   PageD=%d ",key, Tank_nn, PageD);
  flag_prn=0;
}         
...
if(flag_prn)
{
  printf("\n\r END   key=%d   Tank_nn=%d   PageD=%d ",key, Tank_nn, PageD);
  flag_prn=0;
}         
 ...
 */
          ...
          MoveToXY(4,15);   MmiPrintf("%02d.%02d.%02d   %02d:%02d:%02d       " ,day,month,year-2000 ,hour,min,sec);
...
m_pass:
		   ...
		   SetDisplayPage(EmptPage); //EmptPage   // Чистая страница
